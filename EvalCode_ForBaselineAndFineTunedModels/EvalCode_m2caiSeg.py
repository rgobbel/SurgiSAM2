...
def _(mo):
    ...
    Shared eval logic lives in `surgisam_utils` at the project root.
    """)
    return
...
def _():
    ...
    # Ensure the project root (which holds surgisam_utils.py and pyproject.toml)
    # is on sys.path so notebook cells can import shared helpers regardless of
    # the cwd marimo was launched from.
...
@app.cell(hide_code=True)
def _(Path, dataset_root, datasets, mo, split, su):
    root = Path(dataset_root.value).expanduser().resolve()
    pairs_df = su.discover_pairs(root, list(datasets.value), split.value, frame_ext="jpg")
    ...
    if pairs_df.height:
        ...
        summary = _pl.DataFrame(
            schema={"dataset": _pl.String, "class_name": _pl.String, "n_pairs": _pl.UInt32}
        )
...
@app.cell(hide_code=True)
def _(MODEL_PRESETS, Path, load_button, mo, model_preset, sam_configs_dir, su):
...
@app.cell(hide_code=True)
def _(inference_done, mo, pairs_df, root, su):
...
def _(
    ...
    paths_written = su.save_eval_csvs(
        out_dir,
        dataset_prefix="m2caiSeg",
        split=split.value,
