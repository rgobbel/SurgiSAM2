...
class SAM2Train(SAM2Base):
    def __init__(
        ...
        **kwargs,
        ...
        freeze_mask_decoder = kwargs.pop("freeze_sam_mask_decoder", False)
        freeze_prompt_encoder = kwargs.pop("freeze_sam_prompt_encoder", False)
        
        super().__init__(image_encoder, memory_attention, memory_encoder, **kwargs)
        
        if freeze_image_encoder:
            self.freeze_component("image_encoder")
        if freeze_mask_decoder:
            self.freeze_component("sam_mask_decoder")
        if freeze_prompt_encoder:
            self.freeze_component("sam_prompt_encoder")
    ...
    def freeze_component(self, component_name):
      module = getattr(self, component_name, None)
      if module is None:
          raise ValueError(f"Component '{component_name}' not found in the model.")
